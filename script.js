// Variables globales
        ...
        function migrateOldCraftingData(oldItems) {
            ...
            oldItems.forEach(item => {
                ...
                if (['Ornitópteros', 'Vehículos Terrestres', 'Naves', 'Módulos'].includes(item.category)) {
                    vehicles.push(item);
                } else if (['Cuchillos', 'Rifles', 'Pistolas', 'Explosivos', 'Láseres'].includes(item.category)) {
                    weapons.push(item);
                } else {
                    objects.push(item);
                }
        ...
        function renderCraftableItems() {
            ...
            const allCraftable = [...refinedResources, ...objects, ...vehicles, ...weapons].filter(item => {
                if (!item.recipe || item.recipe.length === 0) return false;
                const status = getCraftingStatus(item);
                return status.canCraft;
            });
            ...
            container.innerHTML = allCraftable.slice(0, 6).map(item => {
                const maxCraftable = getMaxCraftable(item);
                let itemType = 'Otro';
                if (refinedResources.includes(item)) itemType = 'Refinado';
                else if (objects.includes(item)) itemType = 'Objeto';
                else if (vehicles.includes(item)) itemType = 'Vehículo';
                else if (weapons.includes(item)) itemType = 'Arma';
            ...
            container.innerHTML = filteredItems.map(item => {
                ...
                return `
                    <div class="item-card crafting-card" data-id="${item.id}">
                        <div class="crafting-status ${craftingStatus.canCraft ? 'can-craft' : 'missing-materials'}">
        ...
        function handleResourceSubmit(e) {
            ...
            const resources = editingType === 'raw' ? rawResources : refinedResources;
        ...
        function addMaterialToRecipe(materialName = '', quantity = 1) {
            const recipeContainer = document.getElementById('recipeContainer');
            const allMaterials = [...rawResources, ...refinedResources];
